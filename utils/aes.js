...
var GibberishAES = (function () {
    var Nr = 14,
        ...
        openSSLKey = function (passwordArr, saltArr) {
            // Number of rounds depends on the size of the AES in use 3 rounds for 256
            //  2 rounds for the key, 1 for the IV 2 rounds for 128        1 round for the
            // key, 1 round for the IV 3 rounds for 192 since it's not evenly divided by 128
            // bits
            var rounds = Nr >= 12
                    ? 3
                    : 2,
                ...
                i;
            md5_hash[0] = GibberishAES
                .Hash
                .MD5(data00);
            result = md5_hash[0];
            for (i = 1; i < rounds; i++) {
                md5_hash[i] = GibberishAES
                    .Hash
...
const AESEncrypt = function(text) {
    ...
    return GibberishAES.aesEncrypt(text, password);
}
// AES解密
const AESDecrypt = function(text, key) {
    if(!text) {
        throw new Error('请输入密文');
    }
    return GibberishAES.aesDecrypt(text, password);
}
